...
def apply_opcode(opcode, one, two, three, data):
    if opcode == 1:
...
def restore_state(data, one, two):
    data[1] = one
    data[2] = two
    return data


def apply_program(data):
    for i in range(0, len(data), 4):
        opcode = data[i]
        if opcode == 99:
            return data
        else:
            data = apply_opcode(*data[i:i + 4], data)
    raise Exception("no opcode 99!")


assert apply_program([1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50]) == [3500, 9, 10, 70, 2, 3, 11, 0,
                                                                     99, 30, 40, 50]
assert apply_program([1, 0, 0, 0, 99]) == [2, 0, 0, 0, 99]
assert apply_program([2, 3, 0, 3, 99]) == [2, 3, 0, 6, 99]
assert apply_program([1, 1, 1, 4, 99, 5, 6, 0, 99]) == [30, 1, 1, 4, 2, 5, 6, 0, 99]
assert apply_program([2, 4, 4, 5, 99, 0]) == [2, 4, 4, 5, 99, 9801]


def solution_one(input_file):
    with open(input_file) as f:
        data = [int(x) for x in f.read().split(',')]
    return apply_program(restore_state(data, 12, 2))


def solution_two(input_file):
    goal = 19690720
    with open(input_file) as f:
        original_input = [int(x) for x in f.read().split(',')]
    for i in range(100):
        for j in range(100):
            test_input = copy(restore_state(original_input, i, j))
            if apply_program(test_input)[0] == goal:
                return i * 100 + j
    return 'not found'


if __name__ == "__main__":
    print(solution_one("../data/day_02.txt")[0])
    print(solution_two("../data/day_02.txt"))
